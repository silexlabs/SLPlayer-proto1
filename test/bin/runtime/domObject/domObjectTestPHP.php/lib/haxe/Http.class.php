...
class haxe_Http {
	...
	public function customRequest($post, $api, $sock, $method) {
		...
		$port = haxe_Http_4($this, $api, $host, $method, $portString, $post, $request, $secure, $sock, $url_regexp);
		$data = null;
		...
		try {
			...
			if($multipart) {
				...
				while($this->file->size > 0) {
					$size = haxe_Http_5($this, $api, $b, $boundary, $buf, $bufsize, $data, $host, $method, $multipart, $port, $portString, $post, $request, $secure, $sock, $uri, $url_regexp);
					$len = 0;
					try {
	...
	public function readHttpResponse($api, $sock) {
		...
		$size = null;
		{
			...
			while($_g < $headers->length) {
				...
				$a = _hx_explode(": ", $hline);
				$hname = $a->shift();
				$hval = haxe_Http_6($this, $_g, $a, $api, $b, $headers, $hline, $hname, $k, $response, $rp, $s, $size, $sock, $status);
		...
		if($size === null) {
			...
			try {
				while($size > 0) {
					$len = $sock->input->readBytes($buf, 0, haxe_Http_7($this, $api, $b, $buf, $bufsize, $chunk_re, $chunked, $headers, $k, $response, $rp, $s, $size, $sock, $status));
					if($chunked) {
						if(!$this->readChunk($chunk_re, $api, $buf, $len)) {
							break;
						}
	...
	static function requestUrl($url) {
		$h = new haxe_Http($url);
		$r = null;
		$h->onData = array(new _hx_lambda(array(&$h, &$r, &$url), "haxe_Http_8"), 'execute');
		$h->onError = array(new _hx_lambda(array(&$h, &$r, &$url), "haxe_Http_9"), 'execute');
		$h->request(false);
		return $r;
	}
...
function haxe_Http_4(&$»this, &$api, &$host, &$method, &$portString, &$post, &$request, &$secure, &$sock, &$url_regexp) {
...
function haxe_Http_5(&$»this, &$api, &$b, &$boundary, &$buf, &$bufsize, &$data, &$host, &$method, &$multipart, &$port, &$portString, &$post, &$request, &$secure, &$sock, &$uri, &$url_regexp) {
...
function haxe_Http_6(&$»this, &$_g, &$a, &$api, &$b, &$headers, &$hline, &$hname, &$k, &$response, &$rp, &$s, &$size, &$sock, &$status) {
	if($a->length === 1) {
		return $a[0];
	} else {
		return $a->join(": ");
...
function haxe_Http_7(&$»this, &$api, &$b, &$buf, &$bufsize, &$chunk_re, &$chunked, &$headers, &$k, &$response, &$rp, &$s, &$size, &$sock, &$status) {
	if($size > $bufsize) {
		return $bufsize;
	} else {
		return $size;
...
function haxe_Http_8(&$h, &$r, &$url, $d) {
	{
		$r = $d;
	}
}
function haxe_Http_9(&$h, &$r, &$url, $e) {
	{
